...
class ColisController extends AbstractController
{
    ...
    if ($cookies->has('auth') && UserController::checkAdmin($cookies->get('auth'),$userRepo)){
        ...
        return $this->render('colis/index.html.twig',[ 'form' =>$form->createView(),
            'coli' => $colis ,'actual_user' => $actual_user,]);
        }else
        {
    ...
    /**
     * @Route("/f", name="fcolis_index",)
     */
    public function fhome(Request $request, ColisRepository $colisRepository )
    {
        $recherche = new Recherche();
        $form = $this->createForm(RechercheType::class,$recherche);
        $form->handleRequest($request);
        ...
        return $this->render('colis/frontcolis.html.twig',[ 'form' =>$form->createView(),
            'coli' => $colis ]);
    }
